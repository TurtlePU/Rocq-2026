...
Racket, Common Lisp, Clojure, Erlang, F* и Rocq.
...
стороны Rocq на самом деле являются аспектами одного и того же базового
механизма &mdash; другими словами, __доказательства &mdash; это программы__.
...
чтения в [файле POSTSCRIPT.md](./POSTSCRIPT.md). Библиографию всех цитируемых
работ можно найти в [файле bibTeX](./bibliography.bib).
...
- Свежая версия Rocq, доступная на
  [домашней странице Rocq](https://rocq-prover.org/install). "Rocq Platform",
...
- IDE для взаимодействия с Rocq.  Есть несколько вариантов:
  ...
  - __VsRocq__ &mdash; это расширение для VS Code, предлагающее простой интерфейс
    в знакомой IDE. Это "рекомендованный по умолчанию" вариант.
    VsRocq может быть использован как обычная IDE, либо может быть совмещён
    с Docker (см. ниже).
...
- Склонируйте к себе репозиторий курса (в VS Code это можно сделать, выбрав
  ...
  `_CoqProject`, наличие которого является триггером для запуска VsRocq.
...
- Убедитесь, что VsRocq работает, дважды щелкнув файл `Basics.v` в списке слева
...
- Если VsRocq не работает, а Вы получаете сообщение об ошибке, что `VsRocqtop`
  ...
  и запустите команду `which VsRocqtop`. Она должна распечатать путь к месту
  установки VsRocq внутри контейнера. Скопируйте этот путь и вставьте его в
  текстовое поле "VsRocq: Path" в настройках расширения VsRocq (доступно
  при нажатии на иконку шестерёнки на странице расширения VsRocq в VSCode),
...
- Чтобы узнать, какие ещё сочетания клавиш доступны, нажмите клавишу F1 и затем
  ...
  [веб-страницу VsRocq](https://github.com/rocq-prover/vsrocq).
...
оригинальной книги (в виде коллекции скриптов на Rocq и файлов HTML) доступен
